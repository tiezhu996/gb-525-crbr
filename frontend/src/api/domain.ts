...
import type { AllergenProfile, AuditEvent, ContactEdge, MatrixResult, ProcessRoute, ProfileUsage, User, VersionDiff } from '@/types/domain'
import type { AssessmentRun, AssessmentStatus } from '@/types/assessment'
import type { ProfileImpactPreview, ProfileImpactPreviewRequest } from '@/types/impact'
...
export const profileApi = {
  ...
  detail: (id: number) => unwrap<{ profile: AllergenProfile; used_by_routes: ProfileUsage[] }>(api.get(`/profiles/${id}`)),
  ...
  impactPreview: (id: number, payload: ProfileImpactPreviewRequest) => unwrap<ProfileImpactPreview>(api.post(`/profiles/${id}/impact-preview`, payload)),
}
